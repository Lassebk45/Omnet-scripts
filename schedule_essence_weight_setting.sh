...
experiment_name="essence_weight_setting_1"
    ...
    for TOPO in "${TOPOS[@]}"; do
        ...
        for M in "${MUTATION_RATE[@]}"; do
            for C in "${CROSSOVER_RATE[@]}"; do
                ES_GENERATE_ID=$(sbatch --parsable essence_generate.sh $TOPO_DIR$TOPO $DEMANDS_DIR${topo_without_type_and_zoo}_0000.yml essence_weight_setting $OMNET_INPUT_FILES_DIR $UPDATE_INTERVAL $TIME_SCALE $ESSENCE_DIR $SCALER $DEMAND_SCALER $WRITE_INTERVAL $OMNET_RESULTS_DIR $FAILURE_SCENARIOS $SYNC_DIR${topo_without_type_and_zoo}/essence_split_${C}_${M} $experiment_name $C $M)
    ...
    for TOPO in "${TOPOS[@]}"; do
        ...
        for M in "${MUTATION_RATE[@]}"; do
            for C in "${CROSSOVER_RATE[@]}"; do
                ...
                #ESSENCE_GENERATE_JOBS="$ESSENCE_GENERATE_JOBS:$ES_GENERATE_ID"
                ES_RUN_ID=$(sbatch --parsable --dependency=afterok$ESSENCE_GENERATE_JOBS essence_execute.sh $TOPO_DIR$TOPO $DEMANDS_DIR${topo_without_type_and_zoo}_0000.yml essence_weight_setting $OMNET_INPUT_FILES_DIR $UPDATE_INTERVAL $TIME_SCALE $ESSENCE_DIR $SCALER $DEMAND_SCALER $WRITE_INTERVAL $OMNET_RESULTS_DIR General $SYNC_DIR${topo_without_type_and_zoo}/essence_split_${C}_${M} $C $M)
                RUN_OMNET_JOBS="$RUN_OMNET_JOBS:$ES_RUN_ID"
                sleep 0.5
            done
        done
